...
#define __attribute__(                                                         \
    x) // only used by libclang plugin, so it only needs to exist under clang
#endif
#endif
...
#define V8TOOLKIT_EXTEND_WRAPPER                                               \
  __attribute__((annotate(V8TOOLKIT_EXTEND_WRAPPER_STRING)))
...
#define V8TOOLKIT_BIDIRECTIONAL_CLASS                                          \
  __attribute__((annotate(V8TOOLKIT_BIDIRECTIONAL_CLASS_STRING)))

/**
 * Annotate the constructor bidirectional should use with this
 */
#define V8TOOLKIT_BIDIRECTIONAL_CONSTRUCTOR                                    \
  __attribute__((annotate(V8TOOLKIT_BIDIRECTIONAL_CONSTRUCTOR_STRING)))

/**
 * Unused, but may come back.
 * Marks a parameter as one that is always the same, not something that will change per instance
 */
#define V8TOOLKIT_BIDIRECTIONAL_INTERNAL_PARAMETER                             \
  __attribute__((annotate(V8TOOLKIT_BIDIRECTIONAL_INTERNAL_PARAMETER_STRING)))
