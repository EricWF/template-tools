...
struct WrappedClass {
  ...
  bool done = false;
  bool valid =
      false; // guilty until proven innocent - don't delete !valid classes because they may be base classes for valid types
  Annotations annotations;
  bool dumped = false; // this class has been dumped to file
  set<WrappedClass *>
      used_classes; // classes this class uses in its wrapped functions/members/etc
        ...
        for (auto &pair : cpp_to_js_type_conversions) {
          if (regex_match(this->type, matches, std::regex(pair.first))) {
            std::cerr << fmt::format("{} matched {}, converting to {}",
                      ...
                      << std::endl;
            auto new_type =
                pair.second; // need a temp because the regex matches point into the current this->type
...
void foreach_constructor(const CXXRecordDecl *klass, Callback &&callback,
  ...
  if (print_logging)
    cerr << "Enumerating constructors for " << klass->getNameAsString()
  ...
  for (CXXMethodDecl *method : klass->methods()) {
    ...
    if (print_logging)
      cerr << "Checking constructor: " << endl;
    ...
    if (get_export_type(constructor) == EXPORT_NONE) {
      if (print_logging)
        cerr << "  Skipping constructor marked for begin skipped" << endl;
      skip = true;
    }
    ...
    if (annotation != "" && !annotations.has(annotation)) {
      ...
      if (skip) {
        if (print_logging)
          cerr << "  Annotation " << annotation
               << " found, but constructor skipped for reason(s) listed above"
               << endl;
      }
    }
    ...
    if (skip) {
      ...
      if (print_logging)
        cerr << " Running callback on constructor" << endl;
...
public:
  BidirectionalBindings(CompilerInstance &compiler_instance,
                        ...
                        WrappedClass &wrapped_class)
      : compiler_instance(compiler_instance),
        js_wrapper_class(js_wrapper_class), wrapped_class(wrapped_class) {}
  ...
  get_all_virtual_methods_for_class(const CXXRecordDecl *klass) {
      ...
      for (CXXMethodDecl *method : current_class->methods()) {
        ...
        if (method->isVirtual() && !method->isPure()) {
          ...
          if (std::find_if(results.begin(), results.end(), [&](auto found) {
                if (get_method_string(compiler_instance, wrapped_class,
                                      method) ==
                    get_method_string(compiler_instance, wrapped_class,
                                      found)) {
  ...
  std::string handle_virtual(const CXXMethodDecl *method) {
    ...
    result << "  JS_ACCESS_" << num_params
           << (method->isConst() ? "_CONST(" : "(");
    ...
    if (num_params > 0) {
      ...
      for (auto &type : types) {
        type_names.push_back(std::regex_replace(
            type.getAsString(), std::regex("\\s*,\\s*"), " V8TOOLKIT_COMMA "));
      }
  ...
  generate_bindings(
      const std::vector<unique_ptr<WrappedClass>> &wrapped_classes) {
    std::stringstream result;
    auto matches =
        wrapped_class.annotations.get_regex("v8toolkit_generate_(.*)");
    if (wrapped_class.annotations.has(V8TOOLKIT_BIDIRECTIONAL_CLASS_STRING)) {
      result << fmt::format(
          "class JS{} : public {}, public v8toolkit::JSWrapper<{}> "
          "{{\npublic:\n", // {{ is escaped {
          short_name(), short_name(), short_name());
      result << fmt::format("    JS{}(v8::Local<v8::Context> context, "
      ...
      foreach_constructor(
          ...
          [&](auto constructor_decl) {
            if (got_constructor) {
              data_error(
                  fmt::format("ERROR: Got more than one constructor for {}",
                              wrapped_class.class_name));
              return;
            }
            got_constructor = true;
            result << get_method_parameters(compiler_instance, wrapped_class,
                                            constructor_decl, true, true);
            constructor_parameter_count = constructor_decl->getNumParams();
            constructor_parameters =
                get_method_param_qual_types(constructor_decl);

          },
          V8TOOLKIT_BIDIRECTIONAL_CONSTRUCTOR_STRING);
      if (!got_constructor) {
        data_error(fmt::format("ERROR: Got no bidirectional constructor for {}",
                               wrapped_class.class_name));
      }
      result << fmt::format(") :\n");
      ...
      auto variable_names =
          generate_variable_names(constructor_parameters, true);
    ...
    auto bidirectional_class_filename =
        fmt::format("v8toolkit_generated_bidirectional_{}.h", short_name());
    bidirectional_class_file.open(bidirectional_class_filename, ios::out);
    if (!bidirectional_class_file) {
      llvm::report_fatal_error(
          fmt::format("Could not open file: {}", bidirectional_class_filename),
          false);
    }
    ...
    for (auto &include : all_base_type_includes) {
      std::cerr << fmt::format(
                       "for bidirectional {}, adding base type include {}",
                       this->short_name(), include)
                << std::endl;
      bidirectional_class_file << "#include " << include << "\n";
    }
...
public:
  ...
  virtual void onEndOfTranslationUnit() override {
    cerr << fmt::format("at end of TU with {} definitions to process",
                        definitions_to_process.size())
         << endl;
    ...
    for (auto wrapped_class_ptr : definitions_to_process) {
      ...
      if (wrapped_class.decl->isDependentType()) {
        cerr << wrapped_class.class_name << endl;
        llvm::report_fatal_error("Trying to post-process dependent type -- "
                                 "this should be filtered in matcher callback");
      }

      cerr << "class annotations: " << join(wrapped_class.annotations.get())
           << endl;
  ...
  std::string handle_data_member(WrappedClass &containing_class,
                                 FieldDecl *field,
                                 const std::string &indentation) {
    std::stringstream result;
    ...
    cerr << "Processing data member for: " << containing_class.name_alias
    ...
    if (export_type != EXPORT_ALL && export_type != EXPORT_EXCEPT) {
      if (PRINT_SKIPPED_EXPORT_REASONS)
        printf("%sSkipping data member %s because not supposed to be exported "
               "%d\n",
               indentation.c_str(), short_field_name.c_str(), export_type);
      return "";
    }

    if (field->getAccess() != AS_public) {
      if (PRINT_SKIPPED_EXPORT_REASONS)
        printf("%s**%s is not public, skipping\n", indentation.c_str(),
    ...
    if (top_level_class->names.count(short_field_name)) {
      data_error(fmt::format("ERROR: duplicate name {}/{} :: {}\n",
                             ...
                             containing_class.class_name, short_field_name));
      return "";
    }
    ...
    if (annotations.has(V8TOOLKIT_READONLY_STRING) ||
      ...
      result << fmt::format(
          ...
          indentation, full_type_name, containing_class.class_name,
          full_field_name, short_field_name);
    } else {
      result << fmt::format(
          "{}class_wrapper.add_member<{}, {}, &{}>(\"{}\");\n", indentation,
          full_type_name, containing_class.class_name, full_field_name,
          short_field_name);
    }
  ...
  std::string handle_method(WrappedClass &klass, CXXMethodDecl *method,
                            const std::string &indentation) {
    ...
    if (print_logging || PRINT_SKIPPED_EXPORT_REASONS)
      cerr << fmt::format("Handling method: {}", full_method_name) << endl;
    ...
    if (export_type != EXPORT_ALL && export_type != EXPORT_EXCEPT) {
      if (PRINT_SKIPPED_EXPORT_REASONS)
        printf("%sSkipping method %s because not supposed to be exported %d\n",
    ...
    if (dyn_cast<CXXConstructorDecl>(method)) {
      if (PRINT_SKIPPED_EXPORT_REASONS)
        printf("%s**skipping constructor %s\n", indentation.c_str(),
               full_method_name.c_str());
      return "";
    }
    if (dyn_cast<CXXDestructorDecl>(method)) {
      if (PRINT_SKIPPED_EXPORT_REASONS)
        printf("%s**skipping destructor %s\n", indentation.c_str(),
               full_method_name.c_str());
      return "";
    }
    ...
    if (method->isVirtual()) {
      fprintf(stderr, "%s :: %s is virtual with %d overrides\n",
              ...
              (int)method->size_overridden_methods());
    } else {
    ...
    if (method->isVirtual() && method->size_overridden_methods()) {
      if (PRINT_SKIPPED_EXPORT_REASONS)
        printf("%s**skipping derived-class override of base class virtual "
               "function %s\n",
               indentation.c_str(), full_method_name.c_str());
      return "";
    }
    ...
    if (dyn_cast<CXXConversionDecl>(method)) {
      if (PRINT_SKIPPED_EXPORT_REASONS)
        cerr << fmt::format("{}**skipping user-defined conversion operator",
                            indentation)
             << endl;
      return "";
    }
    if (print_logging || PRINT_SKIPPED_EXPORT_REASONS)
      cerr << "Method passed all checks" << endl;
    ...
    if (annotations.has(V8TOOLKIT_EXTEND_WRAPPER_STRING)) {
      ...
      if (PRINT_SKIPPED_EXPORT_REASONS)
        cerr << fmt::format("{}**skipping static method marked as v8 class "
                            "wrapper extension method, but will call it during "
                            "class wrapping",
                            indentation)
             << endl;
      top_level_class->wrapper_extension_methods.insert(full_method_name +
                                                        "(class_wrapper);");
    ...
    if (top_level_class->names.count(short_method_name)) {
      data_error(fmt::format("Skipping duplicate name {}/{} :: {}\n",
                             top_level_class->class_name, klass.class_name,
                             short_method_name));
      return "";
    }
    ...
    if (method->isStatic()) {
      ...
      if (static_method_renames.find(short_method_name) !=
          static_method_renames.end()) {
        short_method_name = static_method_renames[short_method_name];
      }

      result << fmt::format(
          "class_wrapper.add_static_method<{}>(\"{}\", &{});\n",
          get_method_return_type_and_parameters(ci, *top_level_class,
                                                klass.decl, method),
      ...
      if (OO_Call == method->getOverloadedOperator()) {
        result << fmt::format("class_wrapper.make_callable<{}>(&{});\n",
                              get_method_return_type_class_and_parameters(
                                  ci, *top_level_class, klass.decl, method),
                              full_method_name);
      } else {
  ...
  void handle_class(
      WrappedClass &
          wrapped_class, // class currently being handled (not necessarily top level)
      bool top_level = true, const std::string &indentation = "") {

    // update the includes for the type for itself -- if it's a templated type, make sure it has includes for the types it is
    //   templated with
    update_wrapped_class_for_type(
        ci, wrapped_class,
        wrapped_class.decl->getTypeForDecl()->getCanonicalTypeInternal());
    PrintLoggingGuard lg;
    if (top_level) {

      if (std::regex_search(wrapped_class.class_name,
                            std::regex("^(class|struct)\\s+v8toolkit"))) {
        lg.log();
      }

      if (!wrapped_class.should_be_wrapped()) {
        cerr << fmt::format("Skipping {}, should_be_wrapped reported false",
                            wrapped_class.class_name)
             << endl;
        return;
      }
      ...
      cerr << "**** In top-level handle_class with type: "
           << wrapped_class.class_name << endl;

      for (auto &ignore_regex : types_to_ignore_regex) {
        if (std::regex_search(wrapped_class.class_name,
                              std::regex(ignore_regex))) {
          cerr << "skipping top level class because it is in "
                  "types_to_ignore_regex list: "
               << wrapped_class.class_name << endl;
      ...
      if (dyn_cast<ClassTemplatePartialSpecializationDecl>(
              wrapped_class.decl)) {
      ...
      if (wrapped_class.decl->getTypeForDecl()->isDependentType()) {
        if (print_logging)
          cerr << "Skipping dependent type top-level class" << endl;
        wrapped_class.valid = false;
        return;
      }
      const ClassTemplateSpecializationDecl *specialization = nullptr;
      if ((specialization = dyn_cast<ClassTemplateSpecializationDecl>(
               wrapped_class.decl)) != nullptr) {
      ...
      if (!is_good_record_decl(wrapped_class.decl)) {
        if (true || print_logging)
          cerr << "Skipping 'bad' CXXRecordDecl" << endl;
        wrapped_class.valid = false;
        return;
      }
      ...
      if (print_logging)
        cerr << "Adding include for class being handled: "
             << wrapped_class.class_name << " : "
             << get_include_for_type_decl(ci, wrapped_class.decl) << endl;

      // get the .h file the type is defined in
      wrapped_class.include_files.insert(
          get_include_for_type_decl(this->ci, wrapped_class.decl));

    } // end if top level class
    else {
      cerr << fmt::format("{} Handling class (NOT top level) {}", indentation,
                          wrapped_class.class_name)
           << endl;
    }
    ...
    auto full_source_loc =
        FullSourceLoc(wrapped_class.decl->getLocation(), this->source_manager);
    ...
    if (top_level) {
      top_level_class->include_files.insert(
          get_include_string_for_fileid(this->ci, file_id));
    }
    ...
    if (top_level) {
      if (print_logging)
        cerr << "About to process methods" << endl;
      for (CXXMethodDecl *method : wrapped_class.decl->methods()) {
        if (method->hasInheritedPrototype()) {
          cerr << fmt::format(
                      "Skipping method {} because it has inherited prototype",
                      method->getNameAsString())
               << endl;
          continue;
        }

        top_level_class->methods.insert(
            handle_method(wrapped_class, method, indentation + "  "));
      }
    }

    if (print_logging)
      cerr << "About to process fields for " << wrapped_class.name_alias
           << endl;
    for (FieldDecl *field : wrapped_class.decl->fields()) {
      top_level_class->members.insert(
          handle_data_member(wrapped_class, field, indentation + "  "));
    }

    if (print_logging)
      cerr << "About to process base class info" << endl;
    ...
    if (annotation_base_type_to_use.size() > 1) {
      data_error(
          fmt::format("More than one base type specified to use for type",
                      wrapped_class.class_name));
    }
    ...
    if (print_logging)
      cerr << "About to process base classes" << endl;
    for (auto base_class : wrapped_class.decl->bases()) {
      ...
      auto base_type_canonical_name =
          get_canonical_name_for_decl(base_type_decl);

      if (base_type_canonical_name == "class v8toolkit::WrappedClassBase" &&
          base_class.getAccessSpecifier() != AS_public) {
        data_error(
            fmt::format("class inherits from v8toolkit::WrappedClassBase but "
                        "not publicly: {}",
                        wrapped_class.class_name)
                .c_str());
      }
      ...
      if (std::find(annotation_base_types_to_ignore.begin(),
                    annotation_base_types_to_ignore.end(),
                    base_type_canonical_name) !=
          annotation_base_types_to_ignore.end()) {
        cerr << "Skipping base type because it was explicitly excluded in "
                "annotation on class: "
             << base_type_name << endl;
        continue;
      } else {
      ...
      if (std::find(base_types_to_ignore.begin(), base_types_to_ignore.end(),
                    base_type_canonical_name) != base_types_to_ignore.end()) {
        cerr << "Skipping base type because it was explicitly excluded in "
                "plugin base_types_to_ignore: "
             << base_type_name << endl;
        continue;
      } else {
        cerr << "Base type was not explicitly excluded via global ignore list"
             << endl;
      }
      if (!annotation_base_type_to_use.empty() &&
          annotation_base_type_to_use[0] != base_type_name) {
        cerr << "Skipping base type because it was not the one specified to "
                "use via annotation: "
             << base_type_name << endl;
        continue;
      }
      ...
      WrappedClass &current_base = get_or_insert_wrapped_class(
          base_record_decl, this->ci, FOUND_BASE_CLASS);

      // if the base type hasn't been independently processed, do that right now
      if (!current_base.done) {
        cerr << fmt::format("interupting handle_class of {} to fully handle "
                            "class of base type: {}",
                            wrapped_class.class_name, current_base.class_name)
             << endl;
      ...
      auto current_base_include =
          get_include_for_type_decl(this->ci, current_base.decl);
      auto current_include =
          get_include_for_type_decl(this->ci, wrapped_class.decl);
    ...
    if (print_logging)
      cerr << "done with base classes" << endl;
    if (must_have_base_type && !found_base_type) {
      data_error(
          fmt::format("base_type_to_use specified but no base type found: {}",
                      wrapped_class.class_name));
    }
    ...
    if (top_level) {
      if (wrapped_class.found_method == FOUND_BASE_CLASS) {
        cerr << fmt::format("Not wrapping constructor for class only wrapped "
                            "because it's base class of another wrapped type")
             << endl;
      } else if (wrapped_class.annotations.has(
                     V8TOOLKIT_DO_NOT_WRAP_CONSTRUCTORS_STRING)) {
        cerr << fmt::format("Not wrapping because class has DO NOT WRAP "
                            "CONSTRUCTORS annotation")
             << endl;
      } else if (wrapped_class.decl->isAbstract()) {
        cerr << "Skipping all constructors because class is abstract: "
             << class_name << endl;
      } else if (wrapped_class.force_no_constructors) {
        cerr << fmt::format("'force no constructors' set on {} so skipping "
                            "making constructors",
                            class_name)
             << endl;
      } else {
        if (print_logging)
          cerr << fmt::format("About to process constructors for {} -- passed "
                              "all checks to skip class constructors.",
                              wrapped_class.class_name)
               << endl;
        ...
        foreach_constructor(wrapped_class.decl, [&](auto constructor) {
          ...
          auto constructor_name_annotation =
              annotations.get_regex(V8TOOLKIT_CONSTRUCTOR_PREFIX "(.*)");
          ...
          if (std::find(used_constructor_names.begin(),
                        used_constructor_names.end(),
                        constructor_name) != used_constructor_names.end()) {
            data_error(fmt::format("Error: because duplicate JS constructor "
                                   "function name: {} in class {}",
                                   constructor_name.c_str(),
                                   wrapped_class.class_name));
            for (auto &name : used_constructor_names) {
              cerr << (fmt::format("Already used constructor name: {}", name))
                   << endl;
            }
          } else {
            cerr << fmt::format("for {}, wrapping constructor {}",
                                wrapped_class.class_name, constructor_name)
                 << endl;
            used_constructor_names.push_back(constructor_name);

            top_level_class->constructors.insert(fmt::format(
                "{}  class_wrapper.add_constructor<{}>(\"{}\", isolate);\n",
                indentation,
                get_method_parameters(this->ci, *top_level_class, constructor),
                constructor_name));
          }
        });
      }
      ...
      //   the static methods
      if (top_level_class->constructors.empty() &&
          top_level_class->has_static_method) {
      ...
      if (wrapped_class.annotations.has(V8TOOLKIT_BIDIRECTIONAL_CLASS_STRING)) {
        ...
        if (print_logging)
          cerr << "Type " << top_level_class->class_name
               << " **IS** bidirectional" << endl;

        auto generated_header_name =
            fmt::format("\"v8toolkit_generated_bidirectional_{}.h\"",
                        top_level_class->get_short_name());

        auto bidirectional_class_name =
            fmt::format("JS{}", top_level_class->get_short_name());
        ...
        cerr << "Creating new Wrapped class object for "
             << bidirectional_class_name << endl;
        auto bidirectional_unique_ptr =
            std::make_unique<WrappedClass>(bidirectional_class_name, this->ci);
        ...
        cerr << fmt::format(
                    ...
                    bidirectional.class_name, wrapped_class.name_alias)
             << endl;
  ...
  virtual void run(const MatchFinder::MatchResult &Result) override {
    ...
    if (const CXXRecordDecl *klass =
            Result.Nodes.getNodeAs<clang::CXXRecordDecl>("not std:: class")) {
      auto class_name = get_canonical_name_for_decl(klass);

      if (klass->isDependentType()) {
        cerr << "Skipping 'class with annotation' dependent type: "
             << class_name << endl;
        return;
      }
      ...
      add_definition(
          get_or_insert_wrapped_class(klass, this->ci, FOUND_UNSPECIFIED));
    }
    ...
    if (const CXXRecordDecl *klass =
      ...
      print_vector(Annotations(klass).get(),
                   "annotations directly on forward declaration");
      ...
      if (klass->isDependentType() && described_tmpl) {
        ...
        annotations_for_class_templates[described_tmpl].merge(
            Annotations(klass));
      }
    ...
    if (const CXXRecordDecl *klass =
      ...
      cerr << endl
           << "Got class derived from v8toolkit::WrappedClassBase: "
           << get_canonical_name_for_decl(klass) << endl;
      if (!is_good_record_decl(klass)) {
        cerr << "skipping 'bad' record decl" << endl;
        return;
      }
      ...
      cerr << "Storing it for later processing (unless dupe)" << endl;
      add_definition(
          get_or_insert_wrapped_class(klass, this->ci, FOUND_INHERITANCE));
    }

    // only pick off the typedefNameDecl entries, but in 3.8, typedefNameDecl() matcher isn't available
    if (auto typedef_decl =
            Result.Nodes.getNodeAs<clang::TypedefNameDecl>("named decl")) {
      ...
      cerr << "qualified name: " << typedef_decl->getQualifiedNameAsString()
           << endl;
      fprintf(stderr, "record decl ptr: %p\n", (void *)record_decl);
      cerr << "include file: "
           << get_include_for_type_decl(this->ci, typedef_decl) << endl;
      ...
      annotations_for_record_decls[record_decl].merge(
          Annotations(typedef_decl));

      if (Annotations(typedef_decl).has(V8TOOLKIT_NAME_ALIAS_STRING)) {
        fprintf(stderr, "Setting alias for %p to %s\n", (void *)record_decl,
                typedef_decl->getNameAsString().c_str());
    ...
    Matcher.addMatcher(
        cxxRecordDecl(allOf(
            unless(isDerivedFrom(
                "::v8toolkit::JSWrapper")), // JS-Classes will be completely regenerated
            ...
            anyOf( // order of these matters.   If a class matches more than one it will only be returned as the first
                ...
                allOf(isDefinition(),
                      cxxRecordDecl(
                          ...
                          .bind("class derived from WrappedClassBase")),

                // mark a type you control with the wrapped class attribute
                allOf(isDefinition(), unless(matchesName("^::std::")),
                      cxxRecordDecl(/*hasAttr(attr::Annotate)*/)
                          .bind("not std:: class")),

                // used for marking types not under your control with the wrapped class attribute
                allOf(unless(isDefinition()), unless(matchesName("^::std::")),
                      cxxRecordDecl(hasAttr(attr::Annotate))
                          .bind("forward declaration with annotation"))))),
        &HandlerForClass);

    Matcher.addMatcher(
        namedDecl(
            allOf(hasAttr(attr::Annotate), // must have V8TOOLKIT_NAME_ALIAS set
...
public:
  ...
  void EndSourceFileAction() {
    ...
    cerr << fmt::format("About to start writing out wrapped classes with {} "
                        "potential classes",
    ...
    while (found_match) {
      ...
      for (auto wrapped_class_iterator = wrapped_classes.begin();
        ...
        cerr << fmt::format("considering dumping class: {}",
                            wrapped_class.class_name)
             << endl;
    ...
    if (generate_v8classwrapper_sfinae) {
      string sfinae_filename = fmt::format(
          "v8toolkit_generated_v8classwrapper_sfinae.h", file_count);
      ofstream sfinae_file;

      sfinae_file.open(sfinae_filename, ios::out);
      if (!sfinae_file) {
        llvm::report_fatal_error(
            fmt::format("Couldn't open {}", sfinae_filename).c_str());
      }

      sfinae_file << "#pragma once\n\n";

      sfinae_file << get_sfinae_matching_wrapped_classes(wrapped_classes)
                  << std::endl;
      sfinae_file.close();
    }

    if (print_logging)
      cerr << "Wrapped " << classes_wrapped << " classes with "
           << methods_wrapped << " methods" << endl;
    cerr << "Classes returned from matchers: " << matched_classes_returned
         << endl;

    cerr << "Classes used that were not wrapped" << endl;

    for (auto &wrapped_class : wrapped_classes) {
      if (!wrapped_class->dumped) {
        continue;
      }
      for (auto used_class : wrapped_class->used_classes) {
        if (!used_class->dumped) {
          cerr << fmt::format("{} uses unwrapped type: {}",
                              wrapped_class->name_alias, used_class->name_alias)
               << endl;
        }
      }
    }
  ...
  void write_classes(int file_count, vector<WrappedClass *> &classes,
                     bool last_one) {

    cerr << fmt::format("writing classes, file_count: {}, classes.size: {}, "
                        "last_one: {}",
                        file_count, classes.size(), last_one)
         << endl;
    // Open file
    string class_wrapper_filename =
        fmt::format("v8toolkit_generated_class_wrapper_{}.cpp", file_count);
    ...
    already_included_this_file.insert(never_include_for_any_file.begin(),
                                      never_include_for_any_file.end());
    ...
    for (WrappedClass *wrapped_class : classes) {
      ...
      if (print_logging)
        cerr << "Dumping " << wrapped_class->class_name << " to file "
             << class_wrapper_filename << endl;
      ...
      include_files.insert(base_type_includes.begin(),
                           base_type_includes.end());

      for (auto &include_file : include_files) {
        //  printf("%s ", include_file.c_str());
        if (include_file != "" &&
            already_included_this_file.count(include_file) == 0) {
    ...
    class_wrapper_file << fmt::format(
        "void v8toolkit_initialize_class_wrappers_{}(v8toolkit::Isolate &); // "
        "may not exist -- that's ok\n",
        file_count + 1);

    if (file_count == 1) {
      class_wrapper_file << fmt::format("void "
                                        "v8toolkit_initialize_class_wrappers("
                                        "v8toolkit::Isolate & isolate) {{\n");

    } else {
      class_wrapper_file << fmt::format("void "
                                        "v8toolkit_initialize_class_wrappers_{}"
                                        "(v8toolkit::Isolate & isolate) {{\n",
                                        file_count);
    }
