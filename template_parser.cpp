...
public:
  ...
  virtual void onEndOfTranslationUnit() override {
    ...
    for (auto wrapped_class_ptr : definitions_to_process) {
      ...
      // all the annotations and name aliases from forward decl's and 'using'
      // typedefs may not have been available
  ...
  std::string handle_data_member(WrappedClass &containing_class,
    ...
    //            }
    //            if (print_logging) cerr << "changing data member from " <<
    //            full_field_name << " to ";
    //
    //            std::string regex_string = fmt::format("{}::{}$",
    //            containing_class->getName().str(), short_field_name);
    //            auto regex = std::regex(regex_string);
    //            auto replacement = fmt::format("{}::{}",
    //            top_level_class_decl->getName().str(), short_field_name);
    //            full_field_name = std::regex_replace(full_field_name, regex,
    //            replacement);
    ...
    // This means that two wrapped classes will count as much towards rolling to
    // the next file as
  ...
  std::string handle_method(WrappedClass &klass, CXXMethodDecl *method,
    ...
    //            if (print_logging) cerr << "changing method name from " <<
    //            full_method_name << " to ";
    //
    //            auto regex = std::regex(fmt::format("{}::{}$",
    //            containing_class->getName().str(), short_method_name));
    //            auto replacement = fmt::format("{}::{}",
    //            top_level_class_decl->getName().str(), short_method_name);
    //            full_method_name = std::regex_replace(full_method_name, regex,
    //            replacement);
    ...
    if (method->isOverloadedOperator()) {
      ...
      if (OO_Call == method->getOverloadedOperator()) {
        // nothing specific to do, just don't skip it only because it's an
        // overloaded operator
      } else {
    ...
    // if (method->isPure()) {
    //     if(!method->isVirtual()) {
    // 	    llvm::report_fatal_error("Got pure non-virtual method - not sure
    // what that even means", false);
    // 	}
    //     if (PRINT_SKIPPED_EXPORT_REASONS) printf("%s**skipping pure virtual
    //     %s\n", indentation.c_str(), full_method_name.c_str());
    //     return "";
    // }

    // If the function is wrapped in derived classes as well, you run into
    // problems where you can't find the right type to
    //   cast the internal field to to find a match for the function type.   You
    //   may only get a Base* when you need to call void(Derived::*)()
    //   so if you only have the virtual function wrapped in Base, you'll always
    //   find the right type of object
  ...
  void handle_class(WrappedClass &wrapped_class, // class currently being
                                                 // handled (not necessarily top
                                                 // level)
                    bool top_level = true,
                    const std::string &indentation = "") {

    // update the includes for the type for itself -- if it's a templated type,
    // make sure it has includes for the types it is
    //   templated with
    ...
    if (top_level) {
      ...
      // if we've gotten this far, then the class passed the tests and should be
      // included
  ...
  virtual void run(const MatchFinder::MatchResult &Result) override {
    ...
    if (const CXXRecordDecl *klass =
      ...
      /*
         ...
         *)pattern, get_canonical_name_for_decl(pattern).c_str());
      ...
      /* check to see if this has any annotations we should associate with its
       * associated template */
    ...
    // only pick off the typedefNameDecl entries, but in 3.8, typedefNameDecl()
    // matcher isn't available
